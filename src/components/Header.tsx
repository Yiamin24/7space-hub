    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
          isScrolled ? 'bg-white' : 'bg-white/98 backdrop-blur-sm'
        }`}
      >
            ...
            <nav className="hidden lg:flex items-center space-x-6 md:space-x-8">
              {navLinks.map((link) => (
                <a
                  ...
                  className="font-paragraph text-base md:text-lg font-medium text-foreground transition-colors hover:text-primary"
                >
                  {link.label}
                </a>
              ))}
